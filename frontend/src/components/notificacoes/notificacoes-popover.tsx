...
export function NotificacoesPopover() {
  ...
  return (
    <div className="relative">
      ...
      {/* Popover com fundo sólido */}
      {open && (
        <div
          className={cn(
            "absolute right-0 top-12 z-50 w-80 rounded-xl border shadow-2xl",
            ...
            {notificacoes.length === 0 ? (
              ...
              notificacoes.map((n: any) => (
                <Link
                  ...
                  href={n.link}
                  onClick={() => setOpen(false)}
                  className="flex items-start gap-3 border-b px-4 py-3 last:border-0 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                >
                  <div className={cn(
